...
export default function CustomerDetailPage() {
  ...
  const load = useCallback(async () => {
    const res = await apiRequest<Customer>(`/customers/${params.id}`)
    if (res.success) {
      setRow(res.data)
      setLoadError(null)
    } else {
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <CardDescription>Update billing details; changes apply to new invoices and POS lookups.</CardDescription>
          </div>
