...
export default function CustomersPage() {
  ...
  const load = useCallback(async () => {
    const res = await apiRequest<Row[]>("/customers")
    if (res.success) {
      setRows(res.data)
      setLoadError(null)
    } else {
  ...
  const handleCreate = form.handleSubmit(async (values) => {
    const res = await apiRequest<Row>("/customers", { method: "POST", body: JSON.stringify(values) })
    if (!res.success) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">GSTIN-ready profiles for billing and POS selection.</p>
        </div>
        <Button onClick={() => handleOpenChange(true)}>New customer</Button>
      </div>
      {loadError ? (
      ...
      <Sheet open={open} onOpenChange={handleOpenChange}>
        <SheetContent className="flex flex-col sm:max-w-md">
          <SheetHeader>
            ...
            <SheetDescription>
              Add a party you can attach at checkout. Only name is required; fill GSTIN when you issue tax invoices.
            </SheetDescription>
          </SheetHeader>
          <Form {...form}>
            <form onSubmit={handleCreate} className="flex flex-1 flex-col gap-4 overflow-y-auto py-4">
              ...
              {INPUT_FIELDS.map((name) => {
                const meta = partyFieldMeta(name, "customer")
                return (
                  <FormField
                    key={name}
                ...
                <TableCell className="text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                      <DropdownMenuLabel className="text-xs font-normal text-muted-foreground">Customer</DropdownMenuLabel>
                      <DropdownMenuSeparator />
